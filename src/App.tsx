      ...
      <main className="flex-1 max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-6 pb-24 md:pb-6 w-full">
        ...
        {activeTab === 'menu' && !activeTrackingOrder && (
          <div className="space-y-8">
            ...
            {/* Elegant PWA Inline Install Banner */}
            {showInstallBannerLocal && !isAdmin && (
              <motion.div
                initial={{ opacity: 0, y: -15 }}
                animate={{ opacity: 1, y: 0 }}
                className="bg-gradient-to-r from-rose-500 via-pink-500 to-rose-600 rounded-3xl p-5 text-white shadow-xl border border-rose-400/20 relative overflow-hidden flex flex-col md:flex-row items-center justify-between gap-5"
                id="inlineInstallBanner"
              >
                {/* Decorative glowing circles */}
                <div className="absolute top-[-30px] right-[-30px] w-36 h-36 bg-white/10 rounded-full blur-2xl" />
                <div className="absolute bottom-[-30px] left-[15%] w-48 h-48 bg-rose-300/10 rounded-full blur-2xl" />

                <div className="flex items-center gap-4 relative z-10">
                  <div className="w-14 h-14 bg-white/15 backdrop-blur-md rounded-2xl flex items-center justify-center text-2xl shadow-inner border border-white/10 shrink-0 select-none">
                    📲
                  </div>
                  <div className="space-y-1">
                    <div className="flex items-center gap-2 flex-wrap">
                      <span className="bg-white/20 text-[9px] font-black tracking-widest px-2.5 py-0.5 rounded-full uppercase">Aplicativo Oficial</span>
                      <span className="bg-amber-400 text-slate-900 text-[9px] font-black tracking-widest px-2.5 py-0.5 rounded-full uppercase animate-pulse">Instalação Direta</span>
                    </div>
                    <h3 className="text-sm md:text-base font-black tracking-tight uppercase">Instale o App na sua Tela Inicial!</h3>
                    <p className="text-xs text-white/85 font-medium leading-relaxed max-w-xl">
                      Acesse a Supreme instantaneamente com um toque na tela do seu celular, sem barra de endereços do navegador e sem gastar memória!
                    </p>
                  </div>
                </div>
                ...
                <div className="flex items-center gap-3 w-full md:w-auto relative z-10 shrink-0">
                  <button
                    type="button"
                    onClick={() => {
                      localStorage.setItem('supreme_install_dismissed', 'true');
                      setShowInstallBannerLocal(false);
                    }}
                    className="px-4 py-3 border border-white/25 hover:bg-white/10 active:scale-95 text-xs font-bold rounded-2xl transition-all cursor-pointer text-center flex-1 md:flex-initial"
                  >
                    Não, obrigado
                  </button>
                  <button
                    ...
                    onClick={async () => {
                      if (deferredPrompt) {
                        ...
                        console.log(`Direct home install outcome: ${outcome}`);
                        if (outcome === 'accepted') {
                        ...
                        // If direct prompt is not supported yet (like iOS Safari or standard browser without prompt ready), trigger custom guide modal
                        setIsCustomInstallModalOpen(true);
                      }
                    }}
                    className="px-6 py-3 bg-white hover:bg-rose-50 active:scale-95 text-rose-600 text-xs font-black uppercase tracking-wider rounded-2xl shadow-md cursor-pointer text-center flex-1 md:flex-initial flex items-center justify-center gap-1.5"
                  >
                    ⚡ Instalar Agora
                  </button>
                </div>
